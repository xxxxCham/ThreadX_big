...
def _backtest_loop_numba(
    ...
    # Reuse logic from MA Crossover (simplified for brevity)
...
class ATRChannelParams:
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "ATRChannelParams":
        return cls(
            atr_window=data.get("atr_window", 14),
...
class ATRChannelStrategy:
    def __init__(self, symbol: str = "UNKNOWN", timeframe: str = "15m", indicator_bank: Any = None):
        self.name = "ATR_Channel"
        self.symbol = symbol
        self.timeframe = timeframe
        self.indicator_bank = indicator_bank

    def generate_signals(self, df: pd.DataFrame, params: dict) -> pd.DataFrame:
        p = ATRChannelParams.from_dict(params)
        df_signals = df.copy()
        
        # ATR Channel Calculation (Keltner-like: EMA +/- ATR*mult)
        ema = df["close"].ewm(span=p.atr_window, adjust=False).mean()
        # Simple ATR calculation for brevity
        high_low = df["high"] - df["low"]
        high_close = (df["high"] - df["close"].shift()).abs()
        low_close = (df["low"] - df["close"].shift()).abs()
        ranges = pd.concat([high_low, high_close, low_close], axis=1)
        true_range = ranges.max(axis=1)
        atr = true_range.ewm(span=p.atr_window, adjust=False).mean()
        
        upper = ema + (atr * p.atr_mult)
        lower = ema - (atr * p.atr_mult)
        
        df_signals["signal"] = "HOLD"
        # Breakout strategy? Or Mean Reversion?
        # Usually Channel Breakout: Close > Upper -> Long
        breakout_up = (df["close"] > upper) & (df["close"].shift(1) <= upper.shift(1))
        df_signals.loc[breakout_up, "signal"] = "ENTER_LONG"
        breakout_down = (df["close"] < lower) & (df["close"].shift(1) >= lower.shift(1))
        df_signals.loc[breakout_down, "signal"] = "ENTER_SHORT"
