...
def set_global_stop(stop: bool = True) -> None:
    ...
    if stop:
        logger.warning("ÔÅ╣´©Å ARR├èT GLOBAL DEMAND├ë")


# Ô£à Fonction worker standalone (picklable pour ProcessPoolExecutor)
def _evaluate_combo_worker(
    ...
    try:
        ...
        # Paramètres spécifiques BB+ATR (ne pas appliquer à MA_Crossover)
...
class SweepRunner:
    ...
    def __init__(
        ...
        self.logger.info(f"­ƒöº Mode parall├®lisme: {mode}")
        ...
        self.logger.info("­ƒÜÇ SweepRunner initialis├® avec IndicatorBank centralis├®")
    ...
    def _calculate_optimal_workers(self) -> int:
        ...
        # V├®rifier RAM disponible (60 GB total ÔåÆ autoriser jusqu'├á 40 workers)
        if PSUTIL_AVAILABLE:
            ...
            try:
                ...
                if gpu_usage_values:
                    ...
                    if gpu_usage_avg < 50 and ram_used_pct < 70:
                        ...
                        self.logger.info(
                            f"Ôåæ Augmentation workers: {current_workers} ÔåÆ {new_workers} (GPU: {gpu_usage_avg:.0f}%, RAM: {ram_used_pct:.0f}%)"
                        )
                        ...
                        self.logger.warning(
                            f"Ôåô R├®duction workers: {current_workers} ÔåÆ {new_workers} (GPU: {gpu_usage_avg:.0f}%, RAM: {ram_used_pct:.0f}%)"
                        )
                        self._last_worker_adjustment = current_batch
                        return new_workers
            except Exception as e:
    ...
    def run_grid(
        ...
        *,
        ...
        Args:
            ...
            strategy_name: Nom de la strat├®gie ├á utiliser
    ...
    def _execute_combinations(
        ...
        *,
        ...
        try:
            ...
            # ├ëvaluation PARALL├êLE des strat├®gies avec ThreadPoolExecutor
                ...
                with executor_class(max_workers=self.max_workers) as executor:
                    ...
                    for batch_idx in range(0, len(combinations), batch_size):
                        ...
                        if self.should_pause or is_global_stop_requested():
                            stop_requested = True
                            self.logger.warning(
                                f"ÔÅ╣´©Å Arr├¬t d├®tect├® avant soumission batch {batch_idx // batch_size}"
                            )
                            break
                        ...
                        for i in range(batch_idx, batch_end):
                            ...
                            # Ô£à Utiliser fonction standalone si ProcessPool, sinon m├®thode self
                    ...
                    # Collecter les r├®sultats au fur et ├á mesure
                    try:
                        for future in as_completed(futures):
                            ...
                            if self.should_pause or is_global_stop_requested():
                                if not stop_requested:
                                    self.logger.warning(
                                        f"ÔÅ╣´©Å Arr├¬t demand├® apr├¿s {completed_count[0]} combos termin├®s"
                                    )
                                    stop_requested = True
                                ...
                                if cancelled_count > 0:
                                    self.logger.warning(
                                        f"ÔÅ╣´©Å {cancelled_count} futures annul├®es en queue"
                                    )
                                break
                            ...
                            try:
                                ...
                                # Mise ├á jour du compteur de progression (thread-safe via GIL)
    ...
    def _execute_combinations_bounded(
        ...
        *,
        ...
        """Ex├®cute les combinaisons avec un flux de t├óches born├® (feed/drain).
        ...
        try:
            ...
            with self._time_stage("strategy_evaluation"):
                ...
                try:
                    feeder_aggr = int(os.getenv("THREADX_FEEDER_AGGR", "10"))
                except Exception:
                    feeder_aggr = 10
    ...
    def _execute_combinations_with_pruning(
        ...
        *,
        ...
        try:
            ...
            # ├ëvaluation avec pruning progressif
        ...
        for combo in combinations:
            ...
            for param_name, param_value in combo.items():
                if param_name.startswith("bb_"):
                    ...
                    # Ô£à MAPPING: bb_windowÔåÆperiod, bb_num_stdÔåÆstd
                    bb_params = {}
                    ...
                    # Ô£à MAPPING: atr_windowÔåÆperiod, atr_methodÔåÆmethod (d├®faut: ema)
                    atr_params = {}
                    for name, value in combo.items():
                        if name == "atr_window":
                        ...
                        # atr_multiplier est pour la STRAT├ëGIE, pas l'indicateur
    ...
    def _normalize_indicator_key(self, indicator_type: str, params: dict) -> str:
        """
        G├®n├¿re cl├® normalis├®e IDENTIQUE ├á celle utilis├®e par _ensure_indicators.
    ...
    def _compute_batch_indicators(
        ...
        *,
        ...
        # Ô£à Validation donn├®es r├®elles
        if real_data is None or real_data.empty:
        ...
        for indicator_type, params_list in unique_indicators.items():
            ...
            if reuse_cache:
                # Ô£à Utilisation du batch_ensure avec vraies donn├®es
                ...
                # Ô£à NORMALISATION: Utiliser M├èME format de cl├®s que _ensure_indicators
                # Ceci ├®vite KeyError ÔåÆ Fallback recalcul ÔåÆ Overhead 16x !!
                for params in params_list:
                    # G├®n├®rer cl├® normalis├®e (IDENTIQUE ├á strategy/bb_atr.py:509)
                    ...
                    if internal_key in batch_results:
                        ...
                        self.logger.debug(f"Ô£à Mapped {indicator_type} {params} ÔåÆ key {normalized_key}")
                    else:
                        self.logger.warning(f"ÔÜá´©Å Missing batch result for {indicator_type} {params}")
    ...
    def _evaluate_single_combination(
        ...
        """
        ├ëvaluation avec VRAI backtest de strat├®gie.
        ...
        try:
            ...
            # Param├¿tres par d├®faut requis
            # Defaults BB+ATR uniquement (pas pour MA_Crossover)
            if strat_name in ["Bollinger_Breakout", "Bollinger_Dual"]:
                if "entry_z" not in strategy_params:
                    strategy_params["entry_z"] = 1.0

            # Ô£à VRAI backtest avec vraies donn├®es + indicateurs pr├®-calcul├®s
            equity_curve, run_stats = strategy.backtest(
                ...
                fee_bps=4.5,
                slippage_bps=0.0,
                precomputed_indicators=computed_indicators,  # ­ƒÜÇ OPTIMISATION: R├®utiliser batch indicators
            )
...
class UnifiedOptimizationEngine:
    ...
    def __init__(
        ...
        self.logger.info(
            "­ƒÜÇ UnifiedOptimizationEngine initialis├® avec IndicatorBank centralis├®"
        )
    ...
    def run_parameter_sweep(self, config: dict, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            with executor_class(max_workers=self.max_workers) as executor:
                ...
                # Soumission des t├óches par BATCH
                self.logger.info(
                ...
                for batch_idx in range(0, len(combinations), batch_size):
                    if self.should_pause or is_global_stop_requested():
                        stop_requested = True
                        self.logger.warning(
                            f"ÔÅ╣´©Å Arr├¬t avant batch {batch_idx // batch_size}"
                        )
                        break
                ...
                try:
                    for future in as_completed(futures):
                        if self.should_pause or is_global_stop_requested():
                            if not stop_requested:
                                self.logger.warning(
                                    f"ÔÅ╣´©Å Arr├¬t apr├¿s {self.completed_combos} combos"
                                )
                                stop_requested = True
                            ...
                            if cancelled > 0:
                                self.logger.warning(f"ÔÅ╣´©Å {cancelled} futures annul├®es")
                            break
    ...
    def _expand_parameter_grid(self, grid_config: dict) -> list[dict]:
        ...
        self.logger.info(
            f"Grille expans├®e: {len(all_combinations)} ÔåÆ {len(unique_combinations)} uniques"
        )
    ...
    def _generate_signals_from_indicator(
        ...
        """G├®n├¿re des signaux de trading ├á partir des r├®sultats d'indicateurs."""
    ...
    def _calculate_performance_metrics(
        ...
        """Calcule les m├®triques de performance ├á partir des signaux."""
